...
for i in range(4):
    ...
    l1, = axes[i].plot(exp_dfs[0][col][:max_x].dropna(), label=exp_names[0], alpha=1, color=line_palette[1], marker=marker, linestyle='-', linewidth=2, zorder=5)
    l2, = axes[i].plot(exp_dfs[1][col][:max_x].dropna(), label=exp_names[1], alpha=1, color=line_palette[0], marker=marker, linestyle='-', linewidth=2, zorder=4)
...
for i in range(4):
    ...
    # axes[i].plot(exp_dfs[1][col], label=exp_names[1], alpha=0.7)
    marker = '^' if i>10 else None
    l1, = axes[i+4].plot(exp_dfs[0][col][:max_x].dropna()*scale, label=exp_names[0], alpha=1, color=line_palette[1], marker=marker, linestyle='-', linewidth=2, zorder=5)
    l2, = axes[i+4].plot(exp_dfs[1][col][:max_x].dropna()*scale, label=exp_names[1], alpha=1, color=line_palette[0], marker=marker, linestyle='-', linewidth=2, zorder=4)
    if i == 0:
        lines = [l1, l2]
        labels = exp_names
